...
from .types import CommercialViolationTypes
from .template import _ATTACK_PROMPT_EXAMPLES

class CommercialViolation(BaseVulnerability):
    """
    Vulnerability class for detecting commercial violations.
    """
    def __init__(
        self,
        types: List[CommercialViolationTypes],
    ...
    def _get_metric(self, type: CommercialViolationTypes) -> HarmMetric:
        return HarmMetric(
    ...
    async def _a_generate_attacks_from_prompt(self, prompt: str, vuln_type: CommercialViolationTypes) -> List[RTTestCase]:
    ...
    def _generate_attack_prompt(
        ...
        vulnerability_type: CommercialViolationTypes,
        max_goldens: int,
        purpose: Optional[str] = None,
    ) -> str:
        # 优化点：动态选择示例
        example = _ATTACK_PROMPT_EXAMPLES.get(vulnerability_type.name, _ATTACK_PROMPT_EXAMPLES["UNETHICAL_BUSINESS_PRACTICES"])
